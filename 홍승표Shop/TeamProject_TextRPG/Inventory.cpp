...
// 지정한 인덱스(슬롯)에 있는 아이템의 가격을 상점에게 알려줍니다.
int Inventory::GetItemPrice(int index) const
{
    // 인덱스가 가방 범위를 벗어나지 않는지 안전하게 검사
    if (index >= 0 && index < static_cast<int>(items_.size()))
    {
        return items_[index]->getPrice(); // Item 객체가 가진 getPrice()를 호출하여 리턴
    }
    return 0; // 잘못된 인덱스면 0골드 반환
}